  ...
  L.Map.addInitHook(function () {
    this.whenReady(function () {
      ...
      if (!('exportError' in this)) {
        this.exportError = {
          wrongBeginSelector:
            'The jQuery selector does not have an initial bracket (.',
          wrongEndSelector:
            'The jQuery selector does not end with a bracket ).',
          jqueryNotAvailable:
            'The options use jQuery selector, but jQuery is not connected. Connect jQuery or use DOM selectors .class, #id or DOM elements.',
          popupWindowBlocked:
            'The print window was blocked by the browser. Please allow pop-ups on this page.',
          emptyFilename:
            'When downloading the map as a file, its name was not specified.',
        };
      }
      ...
      this.export = function (options) {
        ...
        //         // Class selector.
        //         case '.':
        //           var elements = container.getElementsByClassName(
        //             selector.substr(1)
        //           );
        ...
        //           }
        //           jQuerySelector = jQuerySelector.substr(
        //             0,
        //             jQuerySelector.length - 1
        //           );
      ...
      this._printExport = function (options, images) {
        ...
        if (printWindow) {
          ...
          printDocument.write(
            '<html><head><style>@media print { @page { padding: 0; margin: 0; } }</style><title>' +
      ...
      this._downloadExport = function (options) {
        ...
        return exportMethod(options).then(function (result) {
          ...
          if (window.navigator.msSaveOrOpenBlob) {
            ...
            var blobUrl = (window.URL || window.webkitURL).createObjectURL(
              blob
            );
      ...
      this.Export64 = function (options) {
        options = options || {};

        if (Array.isArray(options)) {
          return this._runDownloadTasks(options, 0);
        } else {
          return this._64Export(options);
        }
      };

      this._64Export = function (options) {
        if (!('fileName' in options)) {
          throw new Error(this.exportError.emptyFilename);
        }

        var export64Method = options.export || this.export;

        return export64Method(options).then(function (result) {
          store.setState({
